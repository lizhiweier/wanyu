...
<article class="page-container">
	<form action="/index.php/Admin/Role/role_add" method="post" class="form form-horizontal" id="form-admin-role-add">
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="" id="" name="role_note">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">角色权限：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<?php if(is_array($auth_infoA)): foreach($auth_infoA as $k=>$v): ?><dl class="permission-list">
					<dt>
						<label>
							<input type="checkbox" value="<?php echo ($v["auth_id"]); ?>" name="role_auth_ids[]" id="user-Character-0">
					...
					<dd>
						<dl class="cl permission-list2">
							...
							<dd style="margin-left:0px;">
								<?php if(is_array($auth_infoB)): foreach($auth_infoB as $k1=>$v1): if($v['auth_id'] == $v1['auth_pid']){ ?>
								<label class="">
									<input type="checkbox" value="<?php echo ($v1["auth_id"]); ?>" name="role_auth_ids[]" id="user-Character-0-0-0">
